...
CVibePlus::CVibePlus() :alpha(0), N(MODEL_SIZE), R(20), _min(2), ts(16), patchsize(3), firstFrame(true), needRebuiltModel(true), fgcount(0), updateFactor(5)
{
	for (int i = 0; i < 256; ++i)
	{
...
int CVibePlus::random(int a, int b)
{
	return rand() % (b - a) + a;
	//return rng.uniform(a, b);
}

void CVibePlus::modifyMask(cv::Mat& mask)
{
...
void CVibePlus::initBackgroundModel()
{
...
void CVibePlus::operator()(const cv::Mat& image, cv::Mat& fgmask, double learningRate)
{
...
void CVibePlus::saveBackgroundModels(cv::Mat& image)
{
...
int CVibePlus::getRandomX(int x)
{
...
int CVibePlus::getRandomY(int y)
{
...
void CVibePlus::removeSmallBlobs(cv::Mat& im, double size)
{
...
void CVibePlus::gradientComputation(cv::Mat& res)
{
...
double CVibePlus::getColordist(uchar* p, uchar* q)
{
...
double CVibePlus::getStandardDeviationR(int x, int y)
{
	int mean = 0;
	double res = 0;
	for (int i = 0; i < N; ++i)
