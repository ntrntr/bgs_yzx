...
MyBackgroundSubtractorMOG::MyBackgroundSubtractorMOG(void)
{
...
MyBackgroundSubtractorMOG::MyBackgroundSubtractorMOG(int _history, int _nmixtures, double _backgroundRatio, double _noiseSigma)
{
	frameSize = Size(0, 0);
	frameType = 0;
	nframes = 0;
...
void MyBackgroundSubtractorMOG::operator()(cv::InputArray _image, cv::OutputArray _fgmask, double learningRate)
{
...
void MyBackgroundSubtractorMOG::getBackgroundImage(cv::Mat& image) const
{
...
void MyBackgroundSubtractorMOG::initialize(cv::Size _frameSize, int _frameType)
{
...
void MyBackgroundSubtractorMOG::process8uC1(const cv::Mat& image, cv::Mat& fgmask, double learningRate, cv::Mat& bgmodel, int nmixtures, double backgroundRatio, double varThreshold, double noiseSigma)
{
...
void MyBackgroundSubtractorMOG::process8uC3(const cv::Mat& image, cv::Mat& fgmask, double learningRate, cv::Mat& bgmodel, int nmixtures, double backgroundRatio, double varThreshold, double noiseSigma)
{
	int x, y, k, k1, rows = image.rows, cols = image.cols;
	float alpha = static_cast<float>(learningRate), T = static_cast<float>(backgroundRatio), vT = static_cast<float>(varThreshold);
	int K = nmixtures;
			...
			for (x = 0; x < cols; ++x, mptr += K)
			{
				...
				for (k = 0; k < K; ++k)
				{
					...
					if (d2 < vT * (var[0] + var[1] + var[2]))
					{
						...
						mptr[k].mean = mu + alpha * diff;
						var = Vec3f(
							max(var[0] + alpha*(diff[0] * diff[0] - var[0]),minVar), 
...
void MyBackgroundSubtractorMOG::modifyMask(cv::Mat& mask)
{
...
void MyBackgroundSubtractorMOG::saveImage(cv::Mat& image, std::string& outpath, std::string& filename, int count)
{
	char str[30];
